...
(function($, document) {
	...
	var panelBuffer = '<div class="mui-poppicker">\
		<div class="mui-poppicker-header">\
			...
			<span class="mui-poppicker-top-title"></span>\
			<button class="mui-btn mui-btn-blue mui-poppicker-btn-ok">确定</button>\
			<div class="mui-poppicker-clear"></div>\
		</div>\
		<div class="mui-poppicker-title">\
		</div>\
	...
	var PopPicker = $.PopPicker = $.Class.extend({
		...
		init: function(options) {
			...
			self.topTitle = self.panel.querySelector('.mui-poppicker-top-title');
		...
		setTitle: function(sTitle) {
			var self = this;
			sTitle = sTitle || '';
			if(sTitle != null && sTitle != 'undefined'){
				self.title.style.display = 'block';
				self.title.innerText = sTitle;
				console.log(sTitle);
			}
		},
		//填充title 标题
		setTopTitle: function(sTopTitle) {
			var self = this;
			sTopTitle = sTopTitle || '';
			if(sTopTitle != null && sTopTitle != 'undefined'){
				self.topTitle.innerText = sTopTitle;
			}
		},
		//获取选中的项（数组）
...
(function($, document) {
	...
	var domBuffer = '<div class="mui-dtpicker" data-type="datetime">\
		...
		<div class="mui-dtpicker-title"><h5 data-id="title-y">年</h5><h5 data-id="title-m">月</h5><h5 data-id="title-d">日</h5><h5 data-id="title-h">时</h5><h5 data-id="title-i">分</h5><h5 data-id="title-s">秒</h5></div>\
		<div class="mui-dtpicker-body">\
			...
			<div data-id="picker-s" class="mui-picker">\
				<div class="mui-picker-inner">\
					<div class="mui-pciker-rule mui-pciker-rule-ft"></div>\
					<ul class="mui-pciker-list">\
					</ul>\
	...
	var DtPicker = $.DtPicker = $.Class.extend({
		...
		getSelected: function() {
			...
			switch (type) {
				case 'datetime':
					selected.value = selected.y.value + '-' + selected.m.value + '-' + selected.d.value + ' ' + selected.h.value + ':' + selected.i.value+ ':' + selected.s.value;
					selected.text = selected.y.text + '-' + selected.m.text + '-' + selected.d.text + ' ' + selected.h.text + ':' + selected.i.text + ':' + selected.s.text;
					break;
				case 'date':
					...
					break;
				case 'time':
					selected.value = selected.h.value + ':' + selected.i.value + ':' + selected.s.value;
					selected.text = selected.h.text + ':' + selected.i.text+ ':' + selected.s.text;
					break;
				case 'month':
		...
		setSelectedValue: function(value) {
			...
			if(self.options.type == "time"){
				ui.h.picker.setSelectedValue(parsedValue.h, 0, function() {
					ui.i.picker.setSelectedValue(parsedValue.i, 0, function() {
						ui.s.picker.setSelectedValue(parsedValue.s, 0);
					});
				});
			}else{
		...
		_isBeginMinutes: function() {
			return this._isBeginHours() && this.options.beginMinutes === parseInt(this.ui.i.picker.getSelectedValue());
		},
		_isEndYear: function() {
		...
		_isEndMinutes: function() {
			return this._isEndHours() && this.options.endMinutes === parseInt(this.ui.i.picker.getSelectedValue());
		},
		...
		_createSeconds: function(current) {
			var self = this;
			var options = self.options;
			var ui = self.ui;
			//生成秒列表
			var iArray = [];
			if (options.customData.s) {
				iArray = options.customData.s;
			} else {
				var s = self._isBeginMinutes() ? options.beginSeconds : 0;
				var maxSeconds = self._isEndMinutes() ? options.endSeconds : 59;
				for (; s <= maxSeconds; s++) {
					var val = self._fill(s);
					iArray.push({
						text: val,
						value: val
					});
		...
		_parseValue: function(value) {
			...
			var rs = {};
			if (value) {
				var parts = value.replace(new RegExp(/:/g), "-").replace(" ", "-").split("-");
				if(self.options.type == "time"){
					rs.h = parts[0];
					rs.i = parts[1];
					rs.s = parts[2];
		...
		_create: function(options) {
			var self = this;
			options = options || {};
			options.labels = options.labels || ['年', '月', '日', '时', '分', '秒'];
			...
			if (beginDate instanceof Date && !isNaN(beginDate.valueOf())) { //设定了开始日期
				...
				options.beginSeconds = benginDate.getSeconds();
			}
			...
			if (endDate instanceof Date && !isNaN(endDate.valueOf())) { //设定了结束日期
				...
				options.endSeconds = endDate.getSeconds();
			}
